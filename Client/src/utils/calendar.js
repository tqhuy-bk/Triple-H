...
export const LOOP7 = [0, 1, 2, 3, 4, 5, 6];
export const LOOP6 = [0, 1, 2, 3, 4, 5];
const TK13 = [
...
const LE = {
    solar: [
        ...
        { d: 30, m: 4, i: 'Ngày Giải phóng miền Nam - Thống nhất đất nước' },
        { d: 1, m: 5, i: 'Ngày Quốc tế Lao động' },
        { d: 13, m: 5, i: 'Ngày của mẹ' },
...
export function checkHolidayLunar(dd, mm) {
    var res = LE.lunar.find((item) => item.d === dd && item.m === mm);
    if (res)
        return res.i + ' (' + res.d + '/' + res.m + 'ÂL)';
    else return '';
}

export function checkHolidaySolar(dd, mm) {
    var res = LE.solar.find((item) => item.d === dd && item.m === mm);
    if (res)
        return res.i + ' (' + res.d + '/' + res.m + 'DL)';
    else return '';
}
...
export function getMonth(mm, yy) {
...
export function cellClick(data) {
    if (data === undefined)
        return '';
    var args = data.split(','),
    ...
    var s = {};
    s.lunarDate = getDayString(lunar, sday, smonth, syear) + ' âm lịch)\n';
    s.solarDate = 'Ngày ' + cc[0] + ', tháng ' + cc[1] + ', năm ' + cc[2] + '\n';
    s.startHour = 'Giờ đầu ngày ' + (getCanHour0(jd) + ' ' + CHI[0]) + '\n';
    s.tietKhi = 'Tiết ' + TIETKHI[getSunLongitude(jd + 1, 7.0)] + '\n';
    s.hoangDao = 'Giờ hoàng đạo: ' + getGioHoangDao(jd) + '\n';
